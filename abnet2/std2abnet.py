...
If I ever need to optimize this, start with a formal analysis of the
sampling problem.

Could also be good to write some tests to check there is nothing wrong with
the sampling (visual tests to cope with the randomness ?)
...
def parse_STD_results(STD_file,length_word_max=3):
    """
    INPUT:
    ...
    while i < len(lines):
        ...
        while not(new_class):
            ...
            if len(tokens) == 3:
                ...
                assert tokens == ['']
                new_class = True
                if length_element>length_word_max:
...
def token_sampling_p(std_descr, type_samp = 'f', speaker_samp='f', 
    ...
    Sampling proba modes:
        ...
        -fcube : proportional to square root of type or speaker probabilities
        - 1 : equiprobable
    ...
    elif type_samp == 'fcube':
        W_types = np.cbrt(W_types)
    elif type_samp == '1':
        W_types = list(np.ones(len(W_types)))
    elif type_samp == 'log':
        W_types = np.log([1 + w for w in W_types])
        W_types[W_types == - float('Inf')] = 0
    if speaker_samp == 'f':
        W_speakers = np.sqrt(W_speakers)
    elif speaker_samp == 'fcube':
        W_speakers = np.cbrt(W_speakers)
    elif speaker_samp == '1':
        W_speakers = list(np.ones(len(W_speakers)))
    elif speaker_samp == 'log':
        W_speakers = np.log([1 + w for w in W_speakers])
...
def std2abnet(std_file, spkid_file, train_spk_file, test_spk_file,num_min_char=3,
    ...
    # parsing STD results
    clusters = parse_STD_results(std_file,num_min_char)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--type_sampling_mode', default='f',
                        help = "1, f, f2 or log, fcube, default is f")
    parser.add_argument('--spk_sampling_mode', default='f',
                        help = "1, f,f2 or log, fcube, default is f")
    args = parser.parse_args()
    assert args.type_sampling_mode in ['1', 'f', 'f2','log', 'fcube' ]
    assert args.spk_sampling_mode in ['1', 'f', 'f2','log', 'fcube']
